...
def main():
    ...
    #Print list of successfully parsed MDAs
...
def parse_mda_multiprocess(form_dir: str, mda_dir: str, overwrite: bool = False):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
        for form_path, mda_path in zip(form_paths, mda_paths):
            ParsedSuccessfully, ParseFailed =  parse_mda (form_path, mda_path, ParsedSuccessfully, ParseFailed, overwrite)        
    
    
    print(ParsedSuccessfully)
    print(ParseFailed)
...
def parse_mda(form_path, mda_path,ParsedSuccessfully, ParseFailed, overwrite=False):
    ...
    if mda:
        ...
        ParsedSuccessfully.append(mda_path)

    else:
        print("Parse MDA failed {}".format(form_path))
        ParseFailed.append(form_path)

    return ParsedSuccessfully,ParseFailed
